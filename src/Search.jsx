...
export default function DebtSearch(props) {
  ...
  return (
      <div>
        <TextField InputLabelProps={{style: { color: '#FF10F0', borderColor: '#FF10F0'}}}  label="search" variant='filled' type="text" value={textField} onChange={(e)=>setTextField(e.target.value)}/><Button id="searchButton" onClick={()=>setTheQuery()}>{textField?<SearchIcon sx={{ mr: 2, color: '#FF10F0' }} />:<RefreshIcon sx={{ mr: 2, color: '#FF10F0' }}  />}</Button>
